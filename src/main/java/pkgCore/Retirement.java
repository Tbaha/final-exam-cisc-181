...
public class Retirement {
	...
	public double MonthlySavings() {
		...
		double pmt = Math.abs(Math.round(Retirement.PMT(dAnnualReturnRetired / 12, iYearsRetired * 12,this.TotalAmountToSave(), 0, false) * 100.0) / 100.0);
		return pmt;
	}

	public double TotalAmountToSave() {
		
		double pv = Math.round(Math.abs(Retirement.PV(dAnnualReturnRetired / 12, iYearsRetired * 12, dRequiredIncome - dMonthlySSI, 0, false)));
		return pv;
	}
